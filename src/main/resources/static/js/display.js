...
function displayListFriend(){
    ...
    // danh sách bạn
    let listHTML= `<li class="friend"><div class="nameFriend">Friends list :</div> <button style= "width:30%" onclick="onDisplayPlayout('frmAddFriend','display-none',1)"><i class="fas fa-plus" style="color:black"></i></button></li>`;
    keyFriend.forEach(e=>{
        idNew = e+'friend'
...
function onDisplayListChat(){
    ...
    if (html ===""){
        html = "<h5>The chat list is currently empty</h5>";
        html +="<h6>Choose friends or groups to chat with</h6>"
    }
    onDisplayPlayout('btnAddGroup','display-none',0);
...
function displayListGroup(){
    let listHTML= `<li class="friend"><div class="nameFriend">Your chat group list :</div> <button style="width:30%" onclick="onDisplayPlayout('frmAddGroup','display-none',1)"><i class="fas fa-plus" style="color:black"></i></button></li>`;
...
function displayReceiveListRequest(){
    ...
    listHTML += `<h6 class="friend">Request sent</h6>`; 
        keyRe.forEach(function(e,i){
            listHTML+=`<li class='friend'>
            <div class='picture'></div>
...
function offDislayChat(){
    ...
    selectTitleBox.innerText="Choose friend to chatS";
