...
function insertMessage(mess){   
    ...
    let time = new Date(parseInt(mess.timeStamp));
    ...
    if (status===0) { // status = 0 là gửi
        if (type !== "Image"){
            ...
            BoxChat.innerHTML += `<div class="stl_mes"><img class="send img" src="`+mess.message+`"><br><span>${time}</spa</img></div>`;
        }
    } else {
        if( type !== "Image"){
            BoxChat.innerHTML += `<div class="stl_mes"><span class="receive">${mess.message}<br><span>${time}</span></span></div>`;
        } else{
            BoxChat.innerHTML += `<div class="stl_mes"><img class="receive img" src="`+mess.message+`"><br><span>${time}</spa</img></div>`;
        }
    }
